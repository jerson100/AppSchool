	...
	public void create(Aul_Trabajo t) throws NotCreated {
		...
		try {
			...
			try (Connection db = new ConexionSqlServer().getConexion()) {
				...
				try (PreparedStatement ps = db.prepareStatement("{call dbo.sp_TRABAJOS_ins_upd(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}")) {
	...
	public void delete(Aul_Trabajo t) throws NotDeleted {
		...
		try {
			...
			try (Connection db = new ConexionSqlServer().getConexion()) {
				...
				try (PreparedStatement ps = db.prepareStatement("{call dbo.sp_TRABAJOS_del(?,?,?,?)}")) {
	...
	public List<Aul_Trabajo> all(int idSecCur, int idCuenta) throws NotAll {
		...
		try {
			...
			try (Connection db = new ConexionSqlServer().getConexion()) {
				...
				try (PreparedStatement ps = db.prepareStatement("{call dbo.sp_TRABAJOS_get(?,?)}")) {
